...
int main(int argc, char *argv[])
{
	...
	for (iter = 2; iter <= argc - 1; iter += 2)
	{
        ...
        std::thread sendArpThread(sendArp, ArpHdr::Reply, handle, Mac(send_mac), Mac(send_mac), Mac(att_mac), Ip(tar_ip), Mac(send_mac), Ip(send_ip));
		...
		std::thread spoofThread(spoofProcess, ArpHdr::Reply, handle, send_mac, Mac(send_mac), Mac(att_mac), tar_ip, Mac(send_mac), Ip(send_ip), att_ip);
